...
# Add Python/JS utilities that Claude can import
```
...
# Add API docs, patterns, examples
```

### Skill Design Guidelines

- **Be specific** - Describe exactly when the skill should be used
- **Include workflows** - Step-by-step instructions Claude can follow
- **Provide examples** - Show expected inputs and outputs
- **Handle errors** - Document common failure modes and fixes
- **Use conventional commits** - Maintain consistent commit messages

## API Reference

### GitHubPRClient

The main client class for GitHub PR operations.

```python
from github_pr_resolver.scripts.github_pr_client import GitHubPRClient

client = GitHubPRClient(token="...", repo="owner/repo")
```
...
| Method | Description |
|--------|-------------|
| `get_pull_request(pr_number)` | Fetch PR details |
| `get_review_threads(pr_number)` | Fetch review threads with resolution status |
| `get_check_runs(ref)` | Fetch CI check runs for a commit |
| `resolve_thread(thread_id)` | Mark a review thread as resolved |
| `get_file_content(path, ref)` | Fetch file content from repo |

### get_full_pr_context()

Convenience function that fetches all PR data in one call.

```python
from github_pr_resolver.scripts.github_pr_client import get_full_pr_context

context = get_full_pr_context(
    pr_url_or_number="https://github.com/owner/repo/pull/123",
    token=None,  # Falls back to GITHUB_TOKEN env var
    repo=None    # Extracted from URL
)
```
...
```python
{
    "pr": PullRequest,           # PR details
    "threads": List[dict],       # All review threads
    "checks": List[CheckRun],    # All check runs
    "failing_checks": List[CheckRun],    # Only failing checks
    "unresolved_threads": List[dict],    # Only unresolved threads
    "client": GitHubPRClient,    # Client instance for further calls
